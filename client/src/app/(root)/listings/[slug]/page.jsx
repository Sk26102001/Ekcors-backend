...
function Page() {
    ...
    const handleBooking = async () => {
        ...
        try {
            ...
            const res = await bookMachinery({
                ...
                quantity, // adjust if your API expects quantity
            });
            toast.success(`Booking created. Total: ₹${res?.data?.totalAmount}`);
        } catch (error) {
            console.error(error);
    ...
    const handleAddToCart = () => {
        ...
        const cartItem = {
            id: `${data._id}-${start.toISOString()}-${end.toISOString()}`,
            machineryId: data._id,
            title: data.title,
            image: data.images?.[0] || "/images/default-avatar.jpg",
            quantity,
            startDateTime: start.toISOString(),
            endDateTime: end.toISOString(),
            baseHourlyRate: data.baseHourlyRate,
            dailyCapPrice: data.dailyCapPrice,
            weeklyCapPrice: data.weeklyCapPrice,
            monthlyCapPrice: data.monthlyCapPrice,
        };
        addToCart(cartItem);
    };
        ...
        <section className="bg-neutral-700 md:px-6 px-4">
            <div className="md:py-12 sm:py-8 py-6 max-w-7xl mx-auto grid md:grid-cols-2 md:gap-8 sm:gap-6 gap-4">
                ...
                <div className="md:mt-4 space-y-4">
                    ...
                    <div className="flex justify-evenly text-white text-2xl font-semibold border-y border-neutral-600 py-2">
                        <p>₹{data?.dailyCapPrice || 0}<span className="text-base text-yellowClr">/daily</span></p>
                        <p>₹{data?.weeklyCapPrice || 0}<span className="text-base text-yellowClr">/weekly</span></p>
                        <p>₹{data?.monthlyCapPrice || 0}<span className="text-base text-yellowClr">/monthly</span></p>
                    </div>
                    ...
                    <Card className="border-yellowClr bg-neutral-700 text-white shadow-lg rounded-2xl">
                        <CardHeader>
                            <CardTitle className="text-yellowClr text-xl">Select Rental Time</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            <DateTimePicker
                                label="Start"
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex items-center border border-neutral-600 rounded">
                                    <button
                                        ...
                                        className="px-3 py-1 text-white hover:bg-neutral-700"
                                    >
                                        -
                                    </button>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between text-lg font-semibold">
                                    <span>Total</span>
                                    <span className="text-yellowClr">₹{estimateCost()}</span>
                                </div>
                            </div>

                            <div className="flex gap-3">
                                <Button
                                    className="flex-1 bg-yellowClr text-black hover:bg-yellow-400 transition-all"
                                    onClick={handleBooking}
                                    disabled={loading}
                                >
                                    {loading ? "Processing..." : "Book Now"}
                                ...
                                <Button
                                    variant="outline"
                                    className="flex-1 border-yellowClr text-yellowClr hover:bg-yellowClr/10"
                    ...
                    <Accordion type="single" collapsible className="w-full text-white">
                        <AccordionItem value="item-1">
                            <AccordionTrigger>Equipment Description</AccordionTrigger>
